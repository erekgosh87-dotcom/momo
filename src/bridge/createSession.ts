 ...
 *
 * Used by both `momo remote-control` (empty session so the user has somewhere to
...
export async function createBridgeSession({
  ...
  const { getMomoAIOAuthTokens } = await import('../utils/auth.js')
  ...
  const accessToken =
    getAccessToken?.() ?? getMomoAIOAuthTokens()?.accessToken
  if (!accessToken) {
...
export async function getBridgeSession(
  ...
  const accessToken =
    opts?.getAccessToken?.() ?? getMomoAIOAuthTokens()?.accessToken
  if (!accessToken) {
 ...
 * explicit client action. Both `momo remote-control` (standalone bridge) and the
