...
/**
 * Momo Code parses history for pasted content references to match back to
...
async function* makeLogEntryReader(): AsyncGenerator<LogEntry> {
  ...
  const historyPath = join(getMomoConfigHomeDir(), 'history.jsonl')
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by Momo Code's Tungsten tool.
