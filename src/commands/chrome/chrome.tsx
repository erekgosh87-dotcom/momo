...
type Props = {
  ...
  isMomoAISubscriber: boolean;
  isWSL: boolean;
};
function MomoInChromeMenu(t0) {
  const $ = _c(41);
  const {
    onDone,
  ...
  const isDisabled = isWSL || true && !isMomoAISubscriber;
  let t5;
  if ($[18] !== onDone) {
    t5 = () => onDone();
    $[18] = onDone;
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>Momo in Chrome works with the Chrome extension to let you control your browser directly from Momo Code. Navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">Momo in Chrome is not supported in WSL at this time.</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  let t8;
  if ($[23] !== isMomoAISubscriber) {
    t8 = true && !isMomoAISubscriber && <Text color="error">Momo in Chrome requires a claude.ai subscription.</Text>;
    $[23] = isMomoAISubscriber;
    $[24] = t8;
  } else {
    t8 = $[24];
  }
  ...
  if ($[25] !== handleAction || $[26] !== isConnected || $[27] !== isDisabled || $[28] !== isExtensionInstalled || $[29] !== options || $[30] !== selectKey || $[31] !== showInstallHint) {
    t9 = !isDisabled && <>{!isHomespace && <Box flexDirection="column"><Text>Status:{" "}{isConnected ? <Text color="success">Enabled</Text> : <Text color="inactive">Disabled</Text>}</Text><Text>Extension:{" "}{isExtensionInstalled ? <Text color="success">Installed</Text> : <Text color="warning">Not detected</Text>}</Text></Box>}<Select key={selectKey} options={options} onChange={handleAction} hideIndexes={true} />{showInstallHint && <Text color="warning">Once installed, select {"\"Reconnect extension\""} to connect.</Text>}<Text><Text dimColor={true}>Usage: </Text><Text>momo --chrome</Text><Text dimColor={true}> or </Text><Text>momo --no-chrome</Text></Text><Text dimColor={true}>Site-level permissions are inherited from the Chrome extension. Manage permissions in the Chrome extension settings to control which sites Momo can browse, click, and type on.</Text></>;
    $[25] = handleAction;
    $[26] = isConnected;
    $[27] = isDisabled;
  ...
  let t12;
  if ($[38] !== t11 || $[39] !== t5) {
    t12 = <Dialog title="Momo in Chrome (Beta)" onCancel={t5} color="chromeYellow">{t11}</Dialog>;
    $[38] = t11;
    $[39] = t5;
    $[40] = t12;
  } else {
...
export const call = async function (onDone: (result?: string) => void): Promise<React.ReactNode> {
  ...
  return <MomoInChromeMenu onDone={onDone} isExtensionInstalled={isExtensionInstalled} configEnabled={config.claudeInChromeDefaultEnabled} isMomoAISubscriber={isSubscriber} isWSL={isWSL} />;
};
