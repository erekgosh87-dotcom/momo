...
function Install({
    ...
    async function run() {
      try {
        ...
        if (result.lockFailed) {
          throw new Error('Could not install - another process is currently installing Momo. Please try again in a moment.');
        }
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'Momo Code installation completed successfully', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'Momo Code installation failed', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">
          Installing Momo Code native build {state.version}...
        </Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              Momo Code successfully installed!
            </Text>
          </Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install Momo Code native build',
