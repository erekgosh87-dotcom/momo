...
import { MomoAuthProvider } from '../../services/mcp/auth.js';
import type { McpMomoAIProxyServerConfig, McpHTTPServerConfig, McpSSEServerConfig, McpStdioServerConfig } from '../../services/mcp/types.js';
    ...
    t5 = () => {
      ...
      const prepareServers = async function prepareServers() {
        const serverInfos = await Promise.all(filteredClients.map(async client_0 => {
          ...
          const isMomoAIProxy = client_0.config.type === "claudeai-proxy";
          let isAuthenticated = undefined;
          if (isSSE || isHTTP) {
            const authProvider = new MomoAuthProvider(client_0.name, client_0.config as McpSSEServerConfig | McpHTTPServerConfig);
          ...
          if (isMomoAIProxy) {
            return {
              ...
              config: client_0.config as McpMomoAIProxyServerConfig
            };
          } else {
            if (isSSE) {
              return {
  ...
  if ($[10] !== agentMcpServers.length || $[11] !== filteredClients.length || $[12] !== onComplete || $[13] !== servers.length) {
    t7 = () => {
      ...
      if (servers.length === 0 && agentMcpServers.length === 0) {
        onComplete("No MCP servers configured. Please run /doctor if this is unexpected. Otherwise, run `momo mcp --help` or visit https://code.claude.com/docs/en/mcp to learn more.");
      }
    };
    ...
    case "server-menu":
      {
        ...
        const defaultTab = viewState.server.transport === "claudeai-proxy" ? "claude.ai" : "Momo Code";
