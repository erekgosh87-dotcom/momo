...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getMomoConfigHomeDir(), "MOMO.md");
  const projectMemoryPath = join(getOriginalCwd(), "MOMO.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.claude/MOMO.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./MOMO.md`;
      } else {
        if (file.parent) {
