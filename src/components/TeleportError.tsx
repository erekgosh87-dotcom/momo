...
import { checkIsGitClean, checkNeedsMomoAiLogin } from 'src/utils/background/remote/preconditions.js';
...
export function TeleportError(t0) {
  ...
  const handleLoginWithMomoAI = t6;
  let t7;
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = value => {
      if (value === "login") {
        handleLoginWithMomoAI();
      } else {
        onCancel();
      }
    };
    ...
    case "needsLogin":
      {
        ...
        if ($[16] === Symbol.for("react.memo_cache_sentinel")) {
          t9 = <Box flexDirection="column"><Text dimColor={true}>Teleport requires a Momo.ai account.</Text><Text dimColor={true}>Your Momo Pro/Max subscription will be used by Momo Code.</Text></Box>;
          $[16] = t9;
        } else {
          t9 = $[16];
        }
        ...
        if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
          t10 = <Dialog title="Log in to Momo" onCancel={onCancel}>{t9}<Select options={[{
              label: "Login with Momo account",
              value: "login"
            }, {
              label: "Exit",
              value: "exit"
...
export async function getTeleportErrors(): Promise<Set<TeleportLocalErrorType>> {
  ...
  const [needsLogin, isGitClean] = await Promise.all([checkNeedsMomoAiLogin(), checkIsGitClean()]);
  if (needsLogin) {
