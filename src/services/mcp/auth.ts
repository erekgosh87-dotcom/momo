 ...
 * Used by MomoAuthProvider for metadata discovery and token refresh.
 ...
 *
 * Per RFC 7009, public clients (like Momo Code) should authenticate by including
 ...
 * `momo mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'momo mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
  ...
  try {
    ...
    // (instead of MomoAuthProvider.saveTokens) to avoid instantiating the
...
export async function performMCPOAuthFlow(
  ...
  // rest of CC's transport wiring (MomoAuthProvider.tokens() in client.ts)
  // works unchanged.
  //
  ...
  try {
    ...
    const provider = new MomoAuthProvider(
      serverName,
      serverConfig,
      redirectUri,
      true,
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to Momo Code.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
...
export class MomoAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Momo Code (${this.serverName})`,
   ...
   * but this is the shape MOMO.md flags under "Token/auth caching across
