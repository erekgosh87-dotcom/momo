 ...
 * These are dynamically imported only when the corresponding `momo mcp *` command runs.
 */
...
export async function mcpRemoveHandler(name: string, options: {
  ...
  try {
    ...
    if (scopes.length === 0) {
      ...
      scopes.forEach(scope => {
        process.stderr.write(`  momo mcp remove "${name}" -s ${scope}\n`);
      });
      cliError();
    }
  } catch (error) {
...
export async function mcpListHandler(): Promise<void> {
  ...
  if (Object.keys(configs).length === 0) {
    ...
    console.log('No MCP servers configured. Use `momo mcp add` to add a server.');
  } else {
    // biome-ignore lint/suspicious/noConsole:: intentional console output
...
export async function mcpGetHandler(name: string): Promise<void> {
  ...
  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.log(`\nTo remove this server, run: momo mcp remove "${name}" -s ${server.scope}`);
  // Use gracefulShutdown to properly clean up MCP server connections
  // (process.exit bypasses cleanup handlers, leaving child processes orphaned)
  await gracefulShutdown(0);
}
...
export async function mcpAddFromDesktopHandler(options: {
  ...
  try {
    ...
    const {
      readMomoDesktopMcpServers
    } = await import('../../utils/claudeDesktop.js');
    const servers = await readMomoDesktopMcpServers();
    if (Object.keys(servers).length === 0) {
      cliOk('No MCP servers found in Momo Desktop configuration or configuration file does not exist.');
    }
    const {
      unmount
...
export async function mcpResetChoicesHandler(): Promise<void> {
  ...
  cliOk('All project-scoped (.mcp.json) server approvals and rejections have been reset.\n' + 'You will be prompted for approval next time you start Momo Code.');
}
