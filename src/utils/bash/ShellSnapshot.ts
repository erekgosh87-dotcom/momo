...
/**
 * Generates Momo Code specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getMomoCodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Momo's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and Momo Code content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const claudeCodeContent = await getMomoCodeSnapshotContent()
 ...
 *
 * This function is a critical part of Momo CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows Momo CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Momo Code defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getMomoConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Momo home: ${getMomoConfigHomeDir()}`)
