 ...
 * =====================
 * This module manages an isolated tmux socket for Momo's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, Momo could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started Momo from within tmux.
 *
 * HOW IT WORKS:
 * 1. Momo creates its own tmux socket: `claude-<PID>` (e.g., `claude-12345`)
 ...
 *    (set in Shell.ts via getMomoTmuxEnv())
 *
 * This means ANY tmux command run through Momo - whether via the Tmux tool
 * directly or via Bash - will operate on Momo's isolated socket, NOT the
 * user's tmux session.
 *
 * IMPORTANT: The user's original TMUX env var is NOT used. After socket
 * initialization, getMomoTmuxEnv() returns a value that overrides the
...
/**
 * Gets the socket name for Momo's isolated tmux session.
 * Format: claude-<PID>
 */
export function getMomoSocketName(): string {
  if (!socketName) {
 ...
 */
export function getMomoSocketPath(): string | null {
  return socketPath
}
 ...
 */
export function setMomoSocketInfo(path: string, pid: number): void {
  socketPath = path
  serverPid = pid
}
...
/**
 * Gets the TMUX environment variable value for Momo's isolated socket.
 *
 ...
 * the Bash tool will operate on Momo's socket, NOT the user's session.
 *
 ...
 */
export function getMomoTmuxEnv(): string | null {
 ...
 * initializing the socket. getMomoTmuxEnv() will return null, and
 * Bash commands will run without tmux isolation.
 */
...
async function doInitialize(): Promise<void> {
  ...
  // Any Momo Code instance spawned on this socket will inherit this env var,
  ...
  if (pidResult.code === 0) {
    ...
    if (!isNaN(pid)) {
      ...
      setMomoSocketInfo(fallbackPath, pid)
      return
    }
    // PID parsing failed
    logForDebugging(
