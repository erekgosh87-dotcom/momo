 ...
 * one into the other. Surfaced as warnings by `momo plugin validate` since
...
export async function validatePluginManifest(
  ...
  // them. `momo plugin validate` is a developer tool — authors running it
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Momo Code ` +
  ...
  if (result.success) {
    ...
    // but the Momo.ai marketplace sync rejects non-kebab names. Surfacing
    // this here lets authors catch it in CI before the sync fails on them.
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Momo Code accepts ` +
          `it, but the Momo.ai marketplace sync requires kebab-case ` +
 ...
 * for the load path, but authors running `momo plugin validate` want a hard
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and Momo ` +
