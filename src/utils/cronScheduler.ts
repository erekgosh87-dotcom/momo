...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while Momo
  ...
  function check() {
    ...
    // exists to stop two Momo sessions in the same cwd from double-firing
  ...
  async function enable() {
    ...
    // dies. Prevents double-firing when multiple Momos share a cwd.
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while Momo was not running. ` +
