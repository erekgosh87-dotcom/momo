 ...
 * Handles conversions between Momo's environment and the IDE's environment
 */
...
export interface IDEPathConverter {
  /**
   * Convert path from IDE format to Momo's local format
   * Used when reading workspace folders from IDE lockfile
   */
  toLocalPath(idePath: string): string

  /**
   * Convert path from Momo's local format to IDE format
...
/**
 * Converter for Windows IDE + WSL Momo scenario
 */
