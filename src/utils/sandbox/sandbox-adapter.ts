/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with Momo CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and Momo CLI's
...
import { getMomoTempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve Momo Code-specific path patterns for sandbox-runtime.
 *
 * Momo Code uses special path prefixes in permission rules:
...
/**
 * Convert Momo Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and Momo temp directory as writable
  // The temp directory is needed for Shell.ts cwd tracking files
  const allowWrite: string[] = ['.', getMomoTempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where Momo Code started)
  ...
  // (auto-discovered, auto-loaded, full Momo capabilities) so they need the
  ...
  // core.fsmonitor) escapes the sandbox when Momo's unsandboxed git runs.
  //
 ...
 * Momo's unsandboxed git calls can see them. See the SECURITY block above
 ...
 * This is a Momo CLI-specific function that updates local settings.
 */
...
/**
 * Momo CLI sandbox manager - wraps sandbox-runtime with Momo-specific features
 */
