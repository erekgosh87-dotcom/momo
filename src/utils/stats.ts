...
/**
 * Convert a PersistedStatsCache to MomoCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all Momo Code sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateMomoCodeStats(): Promise<MomoCodeStats> {
 ...
 */
export async function aggregateMomoCodeStatsForRange(
  range: StatsDateRange,
): Promise<MomoCodeStats> {
  if (range === 'all') {
    return aggregateMomoCodeStats()
  }
  ...
  return processedStatsToMomoCodeStats(stats)
}

/**
 * Convert ProcessedStats to MomoCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToMomoCodeStats(
...
function getEmptyStats(): MomoCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
