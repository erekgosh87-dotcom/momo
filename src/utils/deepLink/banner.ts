 ...
 * notice which directory — and therefore which MOMO.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which MOMO.md
 ...
 * and whether its MOMO.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — MOMO.md may be stale' : ''}`,
    )
  }
