 ...
 * Protocol Handler
 *
 * Entry point for `momo --handle-uri <url>`. When the OS invokes claude
 * with a `claude-cli://` URL, this module:
 *   1. Parses the URI into a structured action
 *   2. Detects the user's terminal emulator
 *   3. Opens a new terminal window running momo with the appropriate args
 *
 ...
 * This function parses the URI, resolves the momo binary, and
...
export async function handleDeepLinkUri(uri: string): Promise<number> {
  ...
  // baked at registration time, and we want the terminal-launched Momo to
...
/**
 * Handle the case where momo was launched as the app bundle's executable
...
/**
 * Resolve the working directory for the launched Momo instance.
 * Precedence: explicit cwd > repo lookup (MRU clone) > home.
 * A repo that isn't cloned locally is not an error — fall through to home
 * so a web link referencing a repo the user doesn't have still opens Momo.
 *
