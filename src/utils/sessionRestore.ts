 ...
 * Used for computing initial state before render (per MOMO.md guidelines).
...
export async function processResumedConversation(
  ...
  if (!opts.forkSession) {
    ...
    // the pre-loaded records). Without this seed, `momo -r {newSessionId}`
  ...
  // Compute initial state before render (per MOMO.md guidelines)
