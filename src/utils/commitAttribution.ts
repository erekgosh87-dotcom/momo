...
/**
 * Attribution state for tracking Momo's contributions to files.
 */
...
/**
 * Summary of Momo's contribution for a commit.
 */
...
function computeFileModificationState(
  ...
  try {
    // Calculate Momo's character contribution
...
/**
 * Track a file modification by Momo.
...
/**
 * Track a file creation by Momo (e.g., via bash command).
 * Used when Momo creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by Momo (e.g., via bash rm command).
 * Used when Momo deletes a file through a non-tracked mechanism.
 */
...
export async function calculateCommitAttribution(
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // Momo deleted this file (tracked deletion)
  ...
  for (const result of fileResults) {
    ...
    totalMomoChars += result.claudeChars
  ...
  const totalChars = totalMomoChars + totalHumanChars
  const claudePercent =
    totalChars > 0 ? Math.round((totalMomoChars / totalChars) * 100) : 0
