...
function getSessionsDir(): string {
  return join(getMomoConfigHomeDir(), 'sessions')
}

/**
 * Kind override from env. Set by the spawner (`momo --bg`, daemon
...
/**
 * True when this REPL is running inside a `momo --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `momo ps` sees everything
...
export async function registerSession(): Promise<boolean> {
  ...
  try {
    ...
    // this, the PID file's sessionId goes stale and `momo ps` sparkline
...
/**
 * Push live activity state for `momo ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.claude/sessions/ is shared with Windows-native Momo (symlink
