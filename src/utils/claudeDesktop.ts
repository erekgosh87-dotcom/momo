...
export async function getMomoDesktopConfigPath(): Promise<string> {
  const platform = getPlatform()

  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - Momo Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/Momo/claude_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for Momo Desktop config in each user directory
  ...
  throw new Error(
    'Could not find Momo Desktop config file in Windows. Make sure Momo Desktop is installed on Windows.',
  )
}

export async function readMomoDesktopMcpServers(): Promise<
  Record<string, McpServerConfig>
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - Momo Desktop integration only works on macOS and WSL.',
    )
  }
  try {
    const configPath = await getMomoDesktopConfigPath()
