 ...
 * socket (e.g., "claude-panel-a1b2c3d4") holds the shell session. Each Momo
 ...
 * Momo Code while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per Momo Code instance (based on session ID)
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to Momo Code from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to Momo ', ';',
