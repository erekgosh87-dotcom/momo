...
import { checkAndRefreshOAuthTokenIfNeeded, getMomoAIOAuthTokens } from './auth.js';
...
/**
 * Generates a title and branch name for a coding session using Momo Haiku
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('Momo Code web sessions require authentication with a Momo.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run momo --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run momo --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run momo --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run momo --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
/**
 * Creates a remote Momo.ai session with error handling and UI feedback.
...
/**
 * Creates a remote Momo.ai session using the Sessions API.
 *
 * Two source modes:
