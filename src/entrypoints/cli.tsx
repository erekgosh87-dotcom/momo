...
import { ensureApiKey } from '../utils/momoAuthPrompt.js';
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
      ...
      console.log(`${MACRO.VERSION} (Momo AI Agent)`);
      return;
  }

  // Ensure API key if missing (interactive mode)
  if (!args.includes('-v') && !args.includes('--version') && !args.includes('-p') && !args.includes('--print') && !args.includes('--bare')) {
      await ensureApiKey();
  }
